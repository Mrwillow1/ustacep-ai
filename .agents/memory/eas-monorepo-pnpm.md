...
EAS monorepo builds should source the repository root so the nested Expo app can resolve the root `pnpm-lock.yaml` and `pnpm-workspace.yaml`; set the build job working directory to the app directory rather than creating a second app-local lockfile.
...
**How to apply:** Keep the monorepo root as the EAS source, use the nested app path as the build job working directory, and verify frozen install from that directory before rebuilding.